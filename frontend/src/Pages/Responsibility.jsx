...
const API_URL = import.meta.env.VITE_BACKEND_URL + "/api/edit/responsibility";

const ResponsibilityItem = ({ item, onEdit, onDelete, isLoggedIn }) => (
...
const Responsibility = () => {
  ...
  const fetchResponsibilities = async () => {
    try {
      ...
      setResponsibilities(Array.isArray(res.data) ? res.data : res.data.responsibilities || []);
    } catch (error) {
  ...
  const handleDelete = async (item) => {
    if (window.confirm(`Delete responsibility from ${item.year}?`)) {
      try {
        const token = localStorage.getItem("token");
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        <p className="text-left text-2xl font-mono">Responsibilities</p>
        {isLoggedIn && (
      ...
      {responsibilities.slice(0, visibleCount).map((item) => (
        <ResponsibilityItem key={item._id} item={item} isLoggedIn={isLoggedIn} onEdit={openForm} onDelete={handleDelete} />
      ))}
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-[90%] max-w-md shadow-lg">
            <h2 className="text-lg font-semibold mb-4">{isEditing ? "Edit Responsibility" : "Add Responsibility"}</h2>
